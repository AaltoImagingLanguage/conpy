...
parser.add_argument(
    "subject", metavar="sub###", help="The subject to download the data for"
)
...
download(fname.subject_url(subject=subject), fname.subject_tarball(subject=subject))

# Extract file
makedirs(fname.ds117_dir(subject=subject), exist_ok=True)
print(
    "Unzipping %s -> %s"
    % (fname.subject_tarball(subject=subject), fname.ds117_dir(subject=subject))
)
