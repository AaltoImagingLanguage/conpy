...
for condition in conditions + ["baseline"]:
    ...
    for subject in subjects:
        ...
        stc_fsaverage = stc_subject.morph("fsaverage", subjects_dir=fname.subjects_dir)
        stcs.append(stc_fsaverage)

    # Average the source estimates
    data = np.mean([stc.data for stc in stcs], axis=0)
    ga_stc = mne.SourceEstimate(
        data, vertices=stcs[0].vertices, tmin=stcs[0].tmin, tstep=stcs[0].tstep
    )
