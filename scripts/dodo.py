...
from config import fname, subjects, conditions, freq_bands, bandpass_fmin, bandpass_fmax
...
def task_check():
    ...
    return dict(
        actions=["python check_system.py", "python -m pytest ../conpy/tests/test_*.py"],
    )
...
def task_anatomy():
    ...
    for subject in subjects:
        t1_fname = fname.t1(subject=subject)
        surf_fnames = [
            fname.surface(subject=subject, surf=surf)
...
def task_filter():
    ...
    for subject in subjects:
        sss_fnames = [fname.sss(subject=subject, run=run) for run in range(1, 7)]
        filt_fnames = [
            fname.filt(subject=subject, run=run, fmin=bandpass_fmin, fmax=bandpass_fmax)
...
def task_csd():
    ...
    for subject in subjects:
        epo_fname = fname.epo(subject=subject)
        csd_fnames = [
            fname.csd(subject=subject, condition=cond)
...
def task_power():
    ...
    for subject in subjects:
        ...
        for cond in conditions + ["baseline"]:
            ...
            stc_fnames.append(
                fname.power_hemi(subject=subject, condition=cond, hemi="lh")
            )
            stc_fnames.append(
                fname.power_hemi(subject=subject, condition=cond, hemi="rh")
            )
...
def task_grand_average_power():
    ...
    ga_filenames = [
        fname.ga_power_hemi(condition=cond, hemi="lh")
        for cond in conditions + ["contrast"]
    ]
    ga_filenames += [
        fname.ga_power_hemi(condition=cond, hemi="rh")
        for cond in conditions + ["contrast"]
    ]
...
def task_connectivity_stats():
    ...
    for subject in subjects:
        for cond in conditions:
            con_fnames.append(fname.con(subject=subject, condition=cond))
    ga_con_fnames = [
        fname.ga_con(condition=cond) for cond in conditions + ["contrast", "parcelled"]
    ]
...
def task_figures():
    ...
    yield dict(
        name="forward",
        file_dep=[
            fname.fwd(subject=subjects[0]),
            fname.fwd_r(subject=subjects[0]),
            fname.trans(subject=subjects[0]),
        ],
        targets=["../paper/figures/forward1.png", "../paper/figures/forward2.png"],
    ...
    yield dict(
        name="connectivity",
        file_dep=[
            fname.ga_con(condition="pruned"),
            fname.ga_con(condition="parcelled"),
        ],
        targets=[
            "../paper/figures/degree_lh.png",
