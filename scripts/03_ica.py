...
for run in range(2, 7):
    raw.append(
        mne.io.read_raw_fif(
            fname.filt(
                subject=subject, run=run, fmin=bandpass_fmin, fmax=bandpass_fmax
            ),
            preload=False,
        )
    )
...
ecg_epochs = create_ecg_epochs(raw, tmin=-0.3, tmax=0.3, preload=False)
eog_epochs = create_eog_epochs(raw, tmin=-0.5, tmax=0.5, preload=False)
