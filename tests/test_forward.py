...
from conpy import (restrict_forward_to_vertices,
                   forward_to_tangential, select_shared_vertices,
...
def test_restrict_src_to_vertices(src):
    ...
    # Indices
    src_r = restrict_src_to_vertices(src, [0, 1, 3, src[0]['nuse'],
                                           src[0]['nuse']+1])
    assert_array_equal(src_r[0]['vertno'],
                       src[0]['vertno'][np.array([0, 1, 3])])
    assert_array_equal(src_r[1]['vertno'],
                       src[1]['vertno'][np.array([0, 1])])
    src_r = restrict_src_to_vertices(src, [])
    assert_array_equal(src_r[0]['vertno'], [])
    assert_array_equal(src_r[1]['vertno'], [])
...
def test_restrict_forward_to_vertices(fwd):
    ...
    fwd_fixed = mne.forward.convert_forward_solution(fwd, force_fixed=True,
                                                     use_cps=True)
...
def test_select_shared_vertices(src, fwd):
    # SourceSpaces
    src2 = src.copy()
    src1_r = restrict_src_to_vertices(src, ([14,  54,  59, 108, 228],
                                            [30,  98, 180, 187, 230]))
    ...
    vert_inds = select_shared_vertices([src1_r, src2], ref_src=src2,
                                       subjects_dir=subjects_dir)
    assert_array_equal(vert_inds[0][0], src1_r[0]['vertno'])
    assert_array_equal(vert_inds[0][1], src1_r[1]['vertno'])

    src2_r = restrict_src_to_vertices(src2, [[108, 228, 364, 582, 627],
                                             [187, 230, 263, 271, 313]])
